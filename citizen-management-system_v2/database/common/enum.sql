...
-----------------------------------------------------------------------------------
-- 1. THIẾT LẬP ENUM CHO DATABASE BỘ CÔNG AN
-----------------------------------------------------------------------------------
-- Kết nối đến database Bộ Công an
\connect ministry_of_public_security

-- Tạo function để thiết lập các enum đồng nhất trên các database
...
BEGIN
    ...
    -- Vùng miền
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'geographical_region_type') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'record_type') THEN
        CREATE TYPE record_type AS ENUM ('Chính thức', 'Tạm thời', 'Lịch sử', 'Đang xác minh');
    END IF;
    
    -- Loại tài khoản
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'account_type') THEN
        CREATE TYPE account_type AS ENUM ('Công dân', 'Cán bộ Công an', 'Cán bộ Tư pháp', 'Quản trị hệ thống');
    END IF;
    
    -- Trạng thái duyệt
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'approval_status') THEN
        CREATE TYPE approval_status AS ENUM ('Chờ duyệt', 'Đã duyệt', 'Từ chối', 'Yêu cầu bổ sung');
    END IF;
    
    -- Loại hoạt động
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'activity_type') THEN
        CREATE TYPE activity_type AS ENUM (
            'Đăng nhập', 'Đăng xuất', 'Tạo mới', 'Cập nhật', 'Xóa', 
            'Truy vấn', 'Xuất báo cáo', 'Đồng bộ', 'Phê duyệt', 'Từ chối'
        );
    END IF;
END;
$$ LANGUAGE plpgsql;
...
\echo 'Đã tạo xong các enum cho database Bộ Công an'

-- Xóa hàm tạm sau khi sử dụng xong
DROP FUNCTION create_standard_enums();

-----------------------------------------------------------------------------------
-- 2. THIẾT LẬP ENUM CHO DATABASE BỘ TƯ PHÁP
-----------------------------------------------------------------------------------
...
-- Thiết lập các enum
SELECT create_standard_enums();
\echo 'Đã tạo xong các enum cho database Bộ Tư pháp'

-- Xóa hàm tạm sau khi sử dụng xong
DROP FUNCTION create_standard_enums();

-----------------------------------------------------------------------------------
-- 3. THIẾT LẬP ENUM CHO DATABASE MÁY CHỦ TRUNG TÂM
-----------------------------------------------------------------------------------
...
-- Enum đặc biệt chỉ có ở máy chủ trung tâm
IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'central_sync_type') THEN
...
IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'central_report_type') THEN
    CREATE TYPE central_report_type AS ENUM (
        'Dân số', 'Di cư', 'Hộ tịch', 'Thống kê hành chính', 
        'Điều tra dân số', 'Báo cáo đặc biệt'
    );
END IF;

\echo 'Đã tạo xong các enum cho database máy chủ trung tâm'
