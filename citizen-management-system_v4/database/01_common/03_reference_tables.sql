...
--          Cần quyền tạo bảng trong schema 'reference' trên cả 3 DB.
-- =============================================================================
...
CREATE TABLE reference.regions (
    region_id SMALLINT PRIMARY KEY,                     -- Khóa chính: ID vùng/miền
    region_code VARCHAR(10) NOT NULL UNIQUE,            -- Mã code duy nhất của vùng (vd: BAC, TRUNG, NAM)
    region_name VARCHAR(50) NOT NULL,                   -- Tên vùng/miền (vd: Bắc, Trung, Nam)
    description TEXT,                                   -- Mô tả thêm (nếu cần)
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP, -- Thời gian tạo bản ghi
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP  -- Thời gian cập nhật bản ghi lần cuối
);
COMMENT ON TABLE reference.regions IS '[Chung] Bảng tham chiếu vùng/miền (Bắc, Trung, Nam)';
COMMENT ON COLUMN reference.regions.region_id IS 'ID vùng/miền (vd: 1: Bắc, 2: Trung, 3: Nam)';
...
CREATE INDEX idx_regions_region_code ON reference.regions(region_code); -- Index trên mã code
...
CREATE TABLE reference.provinces (
    province_id INTEGER PRIMARY KEY,                    -- Khóa chính: ID tỉnh/thành phố
    province_code VARCHAR(10) NOT NULL UNIQUE,          -- Mã code duy nhất của tỉnh/thành phố
    province_name VARCHAR(100) NOT NULL,                -- Tên đầy đủ của tỉnh/thành phố
    region_id SMALLINT NOT NULL REFERENCES reference.regions(region_id), -- Khóa ngoại đến vùng/miền
    area DECIMAL(10,2),                                 -- Diện tích (km2, tùy chọn)
    population INTEGER,                                 -- Dân số (ước tính, tùy chọn)
    gso_code VARCHAR(10),                               -- Mã theo Tổng cục Thống kê (tùy chọn)
    is_city BOOLEAN DEFAULT FALSE,                      -- TRUE nếu là Thành phố trực thuộc Trung ương
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
CREATE INDEX idx_provinces_region_id ON reference.provinces(region_id); -- Index trên khóa ngoại
CREATE INDEX idx_provinces_province_code ON reference.provinces(province_code); -- Index trên mã code
CREATE INDEX idx_provinces_province_name ON reference.provinces USING gin (province_name gin_trgm_ops); -- Index GIN cho tìm kiếm tên
...
CREATE TABLE reference.districts (
    district_id INTEGER PRIMARY KEY,                    -- Khóa chính: ID quận/huyện
    district_code VARCHAR(10) NOT NULL UNIQUE,          -- Mã code duy nhất của quận/huyện
    district_name VARCHAR(100) NOT NULL,                -- Tên đầy đủ của quận/huyện
    province_id INTEGER NOT NULL REFERENCES reference.provinces(province_id), -- Khóa ngoại đến tỉnh/thành phố
    area DECIMAL(10,2), population INTEGER, gso_code VARCHAR(10),
    is_urban BOOLEAN DEFAULT FALSE,                     -- TRUE nếu là Quận, Thành phố thuộc tỉnh, Thị xã
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE reference.wards (
    ward_id INTEGER PRIMARY KEY,                        -- Khóa chính: ID phường/xã
    ward_code VARCHAR(10) NOT NULL UNIQUE,              -- Mã code duy nhất của phường/xã
    ward_name VARCHAR(100) NOT NULL,                    -- Tên đầy đủ của phường/xã
    district_id INTEGER NOT NULL REFERENCES reference.districts(district_id), -- Khóa ngoại đến quận/huyện
    area DECIMAL(10,2), population INTEGER, gso_code VARCHAR(10),
    is_urban BOOLEAN DEFAULT FALSE,                     -- TRUE nếu là Phường, Thị trấn
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE reference.ethnicities (
    ...
    ethnicity_code VARCHAR(10) NOT NULL UNIQUE,         -- Mã code duy nhất dân tộc
    ethnicity_name VARCHAR(100) NOT NULL,               -- Tên dân tộc
    description TEXT,
    population INTEGER,                                 -- Dân số ước tính (tùy chọn)
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
...
CREATE TABLE reference.religions (
    ...
    religion_code VARCHAR(10) NOT NULL UNIQUE,          -- Mã code duy nhất tôn giáo
    religion_name VARCHAR(100) NOT NULL,                -- Tên tôn giáo
    description TEXT,
    followers INTEGER,                                  -- Số tín đồ ước tính (tùy chọn)
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE reference.nationalities (
    ...
    nationality_code VARCHAR(10) NOT NULL UNIQUE,       -- Mã quốc tịch theo chuẩn Việt Nam (vd: 001 cho Việt Nam)
    iso_code_alpha2 VARCHAR(2) UNIQUE,                  -- Mã ISO 3166-1 alpha-2 (VN, US, GB...)
    iso_code_alpha3 VARCHAR(3) UNIQUE,                  -- Mã ISO 3166-1 alpha-3 (VNM, USA, GBR...)
    nationality_name VARCHAR(100) NOT NULL,             -- Tên quốc tịch (vd: Việt Nam, Hoa Kỳ)
    country_name VARCHAR(100) NOT NULL,                 -- Tên quốc gia (vd: Cộng hòa xã hội chủ nghĩa Việt Nam)
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE reference.occupations (
    occupation_id INTEGER PRIMARY KEY,                  -- Khóa chính: ID nghề nghiệp
    occupation_code VARCHAR(20) NOT NULL UNIQUE,        -- Mã nghề nghiệp theo chuẩn (VSCO, ISCO...)
    occupation_name VARCHAR(255) NOT NULL,              -- Tên nghề nghiệp
    occupation_group_id INTEGER,                        -- ID nhóm nghề nghiệp (nếu có phân cấp)
    description TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
...
CREATE TABLE reference.authorities (
    authority_id INTEGER PRIMARY KEY,                   -- Khóa chính: ID cơ quan
    authority_code VARCHAR(30) NOT NULL UNIQUE,         -- Mã định danh duy nhất của cơ quan
    authority_name VARCHAR(255) NOT NULL,               -- Tên đầy đủ của cơ quan
    authority_type VARCHAR(100) NOT NULL,               -- Loại cơ quan: Công an, Tư pháp, UBND, Tòa án, Cơ sở y tế...
    address_detail TEXT,                                -- Địa chỉ chi tiết của cơ quan
    ward_id INTEGER REFERENCES reference.wards(ward_id), -- FK đến phường/xã
    district_id INTEGER REFERENCES reference.districts(district_id), -- FK đến quận/huyện
    province_id INTEGER REFERENCES reference.provinces(province_id), -- FK đến tỉnh/thành phố
    phone VARCHAR(50), email VARCHAR(100), website VARCHAR(255),
    parent_authority_id INTEGER REFERENCES reference.authorities(authority_id), -- Cơ quan cấp trên (tự tham chiếu)
...
CREATE TABLE reference.relationship_types (
    ...
    relationship_code VARCHAR(20) NOT NULL UNIQUE,      -- Mã code duy nhất cho quan hệ (vd: VOCHONG, CHACONDE...)
    relationship_name VARCHAR(100) NOT NULL,            -- Tên quan hệ (vd: Vợ-Chồng, Cha đẻ-Con đẻ...)
    inverse_relationship_type_id SMALLINT,              -- ID của quan hệ ngược lại (vd: id của Cha-Con là inverse của Con-Cha)
    description TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
-- Khóa ngoại tự tham chiếu cần DEFERRABLE để tránh lỗi vòng tròn khi insert dữ liệu ban đầu
...
CREATE TABLE reference.prison_facilities (
    prison_facility_id SERIAL PRIMARY KEY,              -- Khóa chính: ID tự tăng
    facility_code VARCHAR(30) UNIQUE,                   -- Mã định danh cơ sở (nếu có)
    facility_name VARCHAR(255) NOT NULL,                -- Tên đầy đủ của cơ sở giam giữ
    facility_type VARCHAR(100),                         -- Loại cơ sở (Trại giam, Trại tạm giam, Nhà tạm giữ...)
    address_detail TEXT,
    ward_id INTEGER REFERENCES reference.wards(ward_id),
    district_id INTEGER REFERENCES reference.districts(district_id),
    province_id INTEGER NOT NULL REFERENCES reference.provinces(province_id), -- Tỉnh/Thành phố nơi đặt cơ sở
    capacity INTEGER,                                   -- Sức chứa (số người, tùy chọn)
