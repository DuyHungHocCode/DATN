-- =============================================================================
-- File: ministry_of_justice/triggers/audit_triggers.sql
-- Description: Tạo function trigger và gắn trigger để ghi nhật ký thay đổi dữ liệu
--              vào bảng audit.audit_log cho các bảng trong database Bộ Tư pháp (BTP).
-- Version: 3.0 (Aligned with Microservices Structure)
--
-- Dependencies:
-- - Schema 'audit' và bảng 'audit.audit_log'.
-- - Các bảng cần được audit trong schema 'justice'.
-- - ENUM 'cdc_operation_type'.
-- =============================================================================

\echo '--> Tạo function và trigger audit cho ministry_of_justice...'
...
-- ============================================================================
-- 1. TẠO TRIGGER FUNCTION (audit.if_modified_func) - GIỐNG HỆT BÊN BCA
--    Đảm bảo hàm này giống hệt hàm đã tạo trong DB BCA để có logic audit nhất quán.
-- ============================================================================
...
-- Danh sách các bảng cần audit trong schema justice
DO $$
DECLARE
    t TEXT;
    trigger_name TEXT;
...
BEGIN
    -- Liệt kê các bảng cần audit trong schema 'justice'
    FOREACH t IN ARRAY ARRAY[
        ...
        'household_member',
        'family_relationship',
        'population_change' -- Audit bảng này nếu nó được cập nhật trực tiếp bởi ứng dụng hoặc trigger khác
    ]
    LOOP
        ...
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'justice' AND table_name = t) THEN
            ...
            RAISE NOTICE '      -> Đã gắn trigger audit cho bảng: justice.%', t;
        ELSE
...
--             EXECUTE format('DROP TRIGGER IF EXISTS %I ON justice.%I;', trigger_name, t);
--             EXECUTE format(
--                 'CREATE TRIGGER %I AFTER TRUNCATE ON justice.%I '
--                 'FOR EACH STATEMENT EXECUTE FUNCTION audit.if_modified_func();', -- Không cần tham số cho TRUNCATE
--                 trigger_name, t
--             );
--             RAISE NOTICE '      -> Đã gắn trigger audit TRUNCATE cho bảng: justice.%', t;
--         END IF;
--     END LOOP;
-- END $$;
...
\echo '-> Hoàn thành tạo function và gắn trigger audit cho ministry_of_justice.'

-- TODO (Các bước tiếp theo):
-- 1. Partition Management for audit_log: Đảm bảo bảng audit_log được phân vùng và quản lý đúng cách.
-- 2. Review Excluded Columns: Xem xét lại danh sách `excluded_columns`.
