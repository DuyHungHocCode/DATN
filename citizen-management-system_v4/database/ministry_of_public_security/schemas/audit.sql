-- =============================================================================
-- File: ministry_of_public_security/schemas/audit.sql
-- Description: Tạo bảng audit_log để ghi nhật ký thay đổi dữ liệu
--              trong schema 'audit' của database Bộ Công an (BCA).
...
\echo '--> Tạo bảng audit.audit_log cho ministry_of_public_security...'
...
CREATE TABLE audit.audit_log (
    ...
    schema_name TEXT NOT NULL,                      -- Schema của bảng bị thay đổi (vd: 'public_security')
    table_name TEXT NOT NULL,                       -- Tên bảng bị thay đổi (vd: 'citizen')
    ...
    application_name TEXT DEFAULT current_setting('application_name'), -- Tên ứng dụng kết nối (vd: 'AppBCA_API')
...
COMMENT ON TABLE audit.audit_log IS '[QLDCQG-BCA] Bảng ghi nhật ký chi tiết các thay đổi dữ liệu (INSERT, UPDATE, DELETE) trên các bảng được theo dõi trong database Bộ Công an.';
...
COMMENT ON COLUMN audit.audit_log.application_name IS 'Tên ứng dụng được đặt khi kết nối (vd: AppBCA_API, psql).';
...
\echo '-> Hoàn thành tạo bảng audit.audit_log cho ministry_of_public_security.'
...
-- 2. Trigger Function: Tạo hoặc đảm bảo tồn tại hàm trigger `audit.if_modified_func()` (trong thư mục `ministry_of_public_security/triggers/audit_triggers.sql`) để ghi dữ liệu vào bảng này. Hàm này cần giống với hàm bên BTP.
-- 3. Attach Triggers: Gắn trigger `audit.if_modified_func()` vào các bảng cần theo dõi trong schema 'public_security' (ví dụ: citizen, address, identification_card...) (trong thư mục `ministry_of_public_security/triggers/audit_triggers.sql`).
-- 4. Permissions: Cấp quyền phù hợp trên bảng này cho role quản trị (security_admin) và có thể là role đọc đặc biệt (trong script `02_security/02_permissions.sql`). Các role ứng dụng thông thường (reader/writer) không nên có quyền ghi trực tiếp vào bảng này.