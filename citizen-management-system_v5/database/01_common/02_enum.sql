...
--              Script này cần chạy trên cả 3 database.
-- Version: 3.2 (Simplified structure, refined comments)
--
-- Yêu cầu: Chạy sau script 01_common/01_extensions.sql.
--          Cần quyền tạo kiểu dữ liệu trong các database.
-- =============================================================================

\echo '*** BẮT ĐẦU QUÁ TRÌNH TẠO ENUM TYPES ***'

-- === 1. TẠO ENUMS CHO DATABASE BỘ CÔNG AN (ministry_of_public_security) ===

\echo '[Bước 1] Tạo ENUMs cho database: ministry_of_public_security...'
...
-- Các ENUM đặc thù của Bộ Công an
...
DROP TYPE IF EXISTS verification_level CASCADE; CREATE TYPE verification_level AS ENUM ('Mức 1', 'Mức 2'); -- Dùng cho VNeID
DROP TYPE IF EXISTS user_type CASCADE; CREATE TYPE user_type AS ENUM ('Công dân', 'Cán bộ', 'Quản trị viên'); -- Loại tài khoản hệ thống
...
-- === 2. TẠO ENUMS CHO DATABASE BỘ TƯ PHÁP (ministry_of_justice) ===

\echo '[Bước 2] Tạo ENUMs cho database: ministry_of_justice...'
...
-- Các ENUM đặc thù của Bộ Tư pháp
...
DROP TYPE IF EXISTS record_status CASCADE; CREATE TYPE record_status AS ENUM ('Mới', 'Đang xử lý', 'Đã duyệt', 'Từ chối', 'Yêu cầu bổ sung', 'Đã hủy', 'Đã cấp'); -- Trạng thái xử lý hồ sơ hộ tịch
...
DROP TYPE IF EXISTS guardianship_type CASCADE; CREATE TYPE guardianship_type AS ENUM ('Giám hộ đương nhiên', 'Giám hộ cử', 'GH cho người chưa thành niên', 'GH cho người mất NLHVDS', 'GH khó khăn nhận thức/hành vi'); -- Loại hình giám hộ
DROP TYPE IF EXISTS recognition_type CASCADE; CREATE TYPE recognition_type AS ENUM ('Cha nhận con', 'Mẹ nhận con', 'Con nhận cha', 'Con nhận mẹ'); -- Loại đăng ký nhận cha mẹ con
DROP TYPE IF EXISTS population_change_type CASCADE; CREATE TYPE population_change_type AS ENUM ('Đăng ký khai sinh', 'Đăng ký khai tử', 'Đăng ký kết hôn', 'Đăng ký ly hôn', 'Đăng ký giám hộ', 'Chấm dứt giám hộ', 'Đăng ký nhận cha mẹ con', 'Đăng ký thay đổi/cải chính/bổ sung hộ tịch', 'Xác định lại dân tộc', 'Xác định lại giới tính', 'Đăng ký thường trú', 'Xóa đăng ký thường trú', 'Đăng ký tạm trú', 'Xóa đăng ký tạm trú', 'Tạm vắng', 'Nhập quốc tịch', 'Thôi quốc tịch', 'Trở lại quốc tịch', 'Khác'); -- Loại biến động dân số/hộ tịch
...
DROP TYPE IF EXISTS household_status CASCADE; CREATE TYPE household_status AS ENUM ('Đang hoạt động', 'Tách hộ', 'Đã chuyển đi', 'Đã xóa', 'Đang cập nhật', 'Lưu trữ'); -- Trạng thái sổ hộ khẩu
DROP TYPE IF EXISTS household_type CASCADE; CREATE TYPE household_type AS ENUM ('Hộ gia đình', 'Hộ tập thể', 'Hộ tạm trú'); -- Loại sổ hộ khẩu

\echo '   -> Hoàn thành tạo ENUMs cho ministry_of_justice.'


-- === 3. TẠO ENUMS CHO DATABASE MÁY CHỦ TRUNG TÂM (national_citizen_central_server) ===

\echo '[Bước 3] Tạo ENUMs cho database: national_citizen_central_server...'
\connect national_citizen_central_server

-- Các ENUM dùng chung (cần định nghĩa lại tại đây để sử dụng)
DROP TYPE IF EXISTS gender_type CASCADE; CREATE TYPE gender_type AS ENUM ('Nam', 'Nữ', 'Khác');
DROP TYPE IF EXISTS blood_type CASCADE; CREATE TYPE blood_type AS ENUM ('A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-', 'Không xác định');
DROP TYPE IF EXISTS death_status CASCADE; CREATE TYPE death_status AS ENUM ('Còn sống', 'Đã mất', 'Mất tích');
DROP TYPE IF EXISTS marital_status CASCADE; CREATE TYPE marital_status AS ENUM ('Độc thân', 'Đã kết hôn', 'Đã ly hôn', 'Góa', 'Ly thân');
...
-- Các ENUM từ BCA/BTP có thể cần dùng trong bảng tích hợp hoặc logic tại TT
DROP TYPE IF EXISTS card_type CASCADE; CREATE TYPE card_type AS ENUM ('CMND 9 số', 'CMND 12 số', 'CCCD', 'CCCD gắn chip');
...
-- (Thêm các ENUM khác từ BCA/BTP nếu cần thiết cho bảng `central.*` hoặc `sync.*`)

-- Các ENUM đặc thù của Máy chủ Trung tâm
...
DROP TYPE IF EXISTS data_source CASCADE; CREATE TYPE data_source AS ENUM ('BCA', 'BTP', 'TT Tổng hợp', 'Nguồn khác'); -- Nguồn gốc dữ liệu tích hợp
DROP TYPE IF EXISTS partition_status CASCADE; CREATE TYPE partition_status AS ENUM ('Hoạt động', 'Đang tạo', 'Đang tách', 'Đang gộp', 'Bảo trì', 'Chỉ đọc', 'Lỗi', 'Đã lưu trữ'); -- Trạng thái partition (dùng trong bảng quản lý partitioning)
DROP TYPE IF EXISTS incident_severity CASCADE; CREATE TYPE incident_severity AS ENUM ('Thông tin', 'Cảnh báo', 'Trung bình', 'Cao', 'Nghiêm trọng'); -- Mức độ sự cố hệ thống
DROP TYPE IF EXISTS incident_type CASCADE; CREATE TYPE incident_type AS ENUM ('Lỗi đồng bộ', 'Lỗi FDW', 'Lỗi ứng dụng TT', 'Lỗi CSDL TT', 'Lỗi hạ tầng', 'Lỗi dữ liệu', 'Lỗi bảo mật', 'Khác'); -- Loại sự cố hệ thống
DROP TYPE IF EXISTS incident_status CASCADE; CREATE TYPE incident_status AS ENUM ('Mới', 'Đang xác minh', 'Đang xử lý', 'Đã xử lý', 'Đã đóng', 'Tạm đóng'); -- Trạng thái xử lý sự cố
...
\echo '-> Bước tiếp theo: Chạy script 01_common/03_reference_tables_structure.sql để tạo cấu trúc bảng tham chiếu.'
