...
-- Table: BTP.Household (Hộ khẩu)
IF OBJECT_ID('BTP.Household', 'U') IS NOT NULL DROP TABLE [BTP].[Household];
GO
PRINT N'  Creating table [BTP].[Household]...';
CREATE TABLE [BTP].[Household] (
    [household_id] BIGINT IDENTITY(1,1) PRIMARY KEY,
    [household_book_no] VARCHAR(20) NOT NULL, -- Số sổ hộ khẩu (nếu còn dùng) hoặc mã định danh hộ
    [head_of_household_id] VARCHAR(12) NOT NULL, -- Logical link to BCA.Citizen.citizen_id (Chủ hộ)
    [address_id] BIGINT NOT NULL, -- Logical link to BCA.Address.address_id (Địa chỉ thường trú của hộ)
    [registration_date] DATE NOT NULL,
    [issuing_authority_id] INT NULL, -- FK to DB_Reference.Reference.Authorities (Cơ quan đăng ký hộ khẩu)
    [area_code] VARCHAR(20) NULL, -- Mã khu vực (nếu có)
    [household_type_id] SMALLINT NOT NULL, -- FK to DB_Reference.Reference.HouseholdTypes
    [household_status_id] SMALLINT NOT NULL, -- FK to DB_Reference.Reference.HouseholdStatuses
    [notes] NVARCHAR(MAX) NULL,
    [created_at] DATETIME2(7) DEFAULT SYSDATETIME(),
    [updated_at] DATETIME2(7) DEFAULT SYSDATETIME()
);
PRINT N'  Table [BTP].[Household] created.';
GO

-- Table: BTP.HouseholdMember (Thành viên hộ khẩu)
IF OBJECT_ID('BTP.HouseholdMember', 'U') IS NOT NULL DROP TABLE [BTP].[HouseholdMember];
GO
PRINT N'  Creating table [BTP].[HouseholdMember]...';
CREATE TABLE [BTP].[HouseholdMember] (
    [household_member_id] BIGINT IDENTITY(1,1) PRIMARY KEY,
    [household_id] BIGINT NOT NULL, -- FK to BTP.Household
    [citizen_id] VARCHAR(12) NOT NULL, -- Logical link to BCA.Citizen.citizen_id (Thành viên)
    [rel_with_head_id] SMALLINT NOT NULL, -- FK to DB_Reference.Reference.RelationshipWithHeadTypes
    [join_date] DATE NOT NULL, -- Ngày tham gia vào hộ
    [leave_date] DATE NULL, -- Ngày rời khỏi hộ
    [leave_reason] NVARCHAR(MAX) NULL,
    [previous_household_id] BIGINT NULL, -- FK to BTP.Household (Hộ khẩu trước đó nếu chuyển đến)
    [member_status_id] SMALLINT NOT NULL, -- FK to DB_Reference.Reference.HouseholdMemberStatuses (e.g., default 'Active')
    [order_in_household] SMALLINT NULL, -- Thứ tự trong sổ hộ khẩu (nếu cần)
    [created_at] DATETIME2(7) DEFAULT SYSDATETIME(),
    [updated_at] DATETIME2(7) DEFAULT SYSDATETIME()
);
PRINT N'  Table [BTP].[HouseholdMember] created.';
GO
