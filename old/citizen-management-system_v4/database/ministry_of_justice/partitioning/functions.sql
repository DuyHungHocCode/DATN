-- File: ministry_of_justice/partitioning/functions.sql
-- Description: Định nghĩa các function PL/pgSQL để hỗ trợ thiết lập và quản lý
--              phân vùng dữ liệu trong database Bộ Tư pháp (BTP).
...
END;
$$ LANGUAGE plpgsql;
COMMENT ON FUNCTION partitioning.setup_nested_partitioning(TEXT, TEXT, TEXT, TEXT, TEXT, BOOLEAN) IS '[QLDCQG-BTP] Thiết lập phân vùng lồng nhau theo Miền->Tỉnh->Huyện (hoặc Miền->Tỉnh) cho bảng, tự động tạo partition con và di chuyển dữ liệu nếu cần.';
...
--              Hữu ích khi thông tin phân vùng của bản ghi thay đổi (vd: nơi đăng ký thay đổi).
...
END;
$$ LANGUAGE plpgsql;
COMMENT ON FUNCTION partitioning.move_data_between_partitions(TEXT, TEXT, TEXT, ANYELEMENT, TEXT, INTEGER, INTEGER) IS '[QLDCQG-BTP] Di chuyển một bản ghi cụ thể sang partition mới dựa trên thông tin phân vùng mới được cung cấp.';
...
END;
$$ LANGUAGE plpgsql;
COMMENT ON FUNCTION partitioning.create_partition_indexes(TEXT, TEXT, TEXT[]) IS '[QLDCQG-BTP] Tạo các index được định nghĩa trong mảng p_index_definitions trên tất cả các partition con của bảng p_table.';
...
BEGIN
    ...
    -- (Tương tự như bên BCA, áp dụng cho bảng audit_log của BTP nếu cần)
...
END;
$$ LANGUAGE plpgsql;
COMMENT ON FUNCTION partitioning.archive_old_partitions(TEXT, TEXT, INTERVAL) IS '[QLDCQG-BTP] Placeholder - Xử lý tách và lưu trữ các partition cũ (cần triển khai logic cụ thể, ví dụ cho audit_log).';

COMMIT;

\echo '-> Hoàn thành định nghĩa functions phân vùng cho ministry_of_justice.'

-- TODO (Các bước tiếp theo):
-- 1. Execute Setup: Tạo file `execute_setup.sql` trong cùng thư mục để gọi các hàm setup (vd: `setup_nested_partitioning`) cho các bảng cụ thể trong DB BTP.
