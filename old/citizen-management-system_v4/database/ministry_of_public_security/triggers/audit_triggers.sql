-- File: ministry_of_public_security/triggers/audit_triggers.sql
-- Description: Tạo function trigger và gắn trigger để ghi nhật ký thay đổi dữ liệu
--              vào bảng audit.audit_log cho các bảng trong database Bộ Công an (BCA).
-- Version: 3.0 (Aligned with Microservices Structure)
--
-- Dependencies:
-- - Schema 'audit' và bảng 'audit.audit_log'.
-- - Các bảng cần được audit trong schema 'public_security'.
-- - ENUM 'cdc_operation_type'.
-- =============================================================================

\echo '--> Tạo function và trigger audit cho ministry_of_public_security...'
...
-- ============================================================================
-- 1. TẠO TRIGGER FUNCTION (audit.if_modified_func)
-- ============================================================================
...
-- ============================================================================
\echo '    -> Gắn trigger audit vào các bảng trong public_security...';

-- Danh sách các bảng cần audit trong schema public_security
DO $$
DECLARE
    t TEXT;
    trigger_name TEXT;
    -- Các cột thường không cần log giá trị cũ/mới chi tiết
    -- Có thể bổ sung thêm các cột khác nếu cần
    excluded_columns TEXT[] := ARRAY['created_at', 'updated_at', 'created_by', 'updated_by', 'password_hash', 'two_factor_secret', 'verification_token', 'password_reset_token'];
BEGIN
    FOR t IN
        SELECT table_name FROM information_schema.tables
        WHERE table_schema = 'public_security' AND table_type = 'BASE TABLE'
          AND table_name NOT LIKE '%_old' -- Loại bỏ các bảng backup cũ nếu có
          AND table_name NOT IN ('spatial_ref_sys') -- Loại bỏ bảng của PostGIS
    LOOP
...
--         EXECUTE format('DROP TRIGGER IF EXISTS %I ON public_security.%I;', trigger_name, t);
--         EXECUTE format(
--             'CREATE TRIGGER %I AFTER TRUNCATE ON public_security.%I '
--             'FOR EACH STATEMENT EXECUTE FUNCTION audit.if_modified_func();', -- Không cần tham số cho TRUNCATE
--             trigger_name, t
--         );
--         RAISE NOTICE '      -> Đã gắn trigger audit TRUNCATE cho bảng: public_security.%', t;
--     END LOOP;
-- END $$;

COMMIT;

\echo '-> Hoàn thành tạo function và gắn trigger audit cho ministry_of_public_security.'

-- TODO (Các bước tiếp theo):
-- 1. Partition Management for audit_log: Đảm bảo bảng audit_log được phân vùng và quản lý đúng cách.
-- 2. Review Excluded Columns: Xem xét lại danh sách `excluded_columns` cho phù hợp với yêu cầu bảo mật và dung lượng log.
-- 3. Performance Monitoring: Theo dõi hiệu năng của trigger, đặc biệt trên các bảng có tần suất ghi cao.