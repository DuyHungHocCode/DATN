-- File: ministry_of_public_security/partitioning/functions.sql
-- Description: Định nghĩa các function PL/pgSQL để hỗ trợ thiết lập và quản lý
--              phân vùng dữ liệu trong database Bộ Công an (BCA).
...
-- - Schema 'public_security' và các bảng cần phân vùng (citizen, address...).
-- - Extension 'unaccent' (cho việc chuẩn hóa tên partition).
-- =============================================================================
...
-- ============================================================================
-- I. FUNCTIONS HELPER (Hàm trợ giúp)
-- ============================================================================
...
END;
$$ LANGUAGE plpgsql;
COMMENT ON FUNCTION partitioning.log_history(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, BIGINT, BIGINT) IS 'Hàm tiện ích để ghi log vào bảng partitioning.history.';
...
COMMENT ON FUNCTION partitioning.standardize_name(TEXT) IS 'Chuẩn hóa tên (bỏ dấu, khoảng trắng->_, lowercase) dùng cho tên partition.';
...
COMMENT ON FUNCTION partitioning.pg_get_tabledef(TEXT, TEXT) IS 'Lấy định nghĩa cột cơ bản của bảng (trừ cột khóa phân vùng cha) để tạo partition con.';


-- ============================================================================
-- II. FUNCTIONS SETUP PHÂN VÙNG CHÍNH
-- ============================================================================
...
END;
$$ LANGUAGE plpgsql;
COMMENT ON FUNCTION partitioning.setup_nested_partitioning(TEXT, TEXT, TEXT, TEXT, TEXT, BOOLEAN) IS '[QLDCQG-BCA] Thiết lập phân vùng lồng nhau theo Miền->Tỉnh->Huyện (hoặc Miền->Tỉnh) cho bảng, tự động tạo partition con và di chuyển dữ liệu nếu cần.';


-- Function: partitioning.determine_partition_for_citizen
-- Description: Xác định thông tin phân vùng (Miền, Tỉnh, Huyện) cho một công dân
--              dựa trên địa chỉ thường trú hoặc tạm trú hiện tại của họ.
-- Arguments: p_citizen_id TEXT
-- Returns: TABLE (geographical_region TEXT, province_id INTEGER, district_id INTEGER)
-- Requires: Bảng public_security.citizen, permanent_residence, temporary_residence, address và reference.provinces tồn tại.
-- ----------------------------------------------------------------------------
...
BEGIN
    -- 1. Tìm địa chỉ thường trú hiện tại (status = TRUE)
    ...
    ORDER BY pr.registration_date DESC, pr.permanent_residence_id DESC -- Lấy bản ghi mới nhất
    LIMIT 1;

    -- 2. Nếu không có thường trú, tìm địa chỉ tạm trú hiện tại (status = 'Active')
    IF v_address_id IS NULL THEN
        ...
        ORDER BY tr.registration_date DESC, tr.temporary_residence_id DESC -- Lấy bản ghi mới nhất
        LIMIT 1;
    END IF;

    -- 3. Nếu tìm được địa chỉ (thường trú hoặc tạm trú), lấy thông tin phân vùng từ bảng address và provinces
    IF v_address_id IS NOT NULL THEN
        ...
        IF FOUND THEN -- Đảm bảo join thành công
            ...
            SELECT
                CASE v_region_id
                    WHEN 1 THEN 'Bắc'::TEXT
                    WHEN 2 THEN 'Trung'::TEXT
                    WHEN 3 THEN 'Nam'::TEXT
                    ELSE NULL::TEXT -- Trường hợp không xác định được vùng
                END AS geographical_region,
                v_province_id,
                v_district_id;
            RETURN; -- Kết thúc hàm nếu tìm thấy
        ELSE
             RAISE NOTICE '[determine_partition_for_citizen] Không tìm thấy thông tin tỉnh/huyện/vùng cho address_id: %', v_address_id;
        END IF;
    ELSE
         RAISE NOTICE '[determine_partition_for_citizen] Không tìm thấy địa chỉ thường trú hoặc tạm trú hiện tại cho citizen_id: %', p_citizen_id;
    END IF;

    -- 4. Trả về NULL nếu không thể xác định
    RAISE NOTICE '[determine_partition_for_citizen] Không thể xác định phân vùng cho citizen_id: %. Trả về NULL.', p_citizen_id;
    RETURN QUERY SELECT NULL::TEXT, NULL::INTEGER, NULL::INTEGER WHERE FALSE; -- Trả về tập rỗng

END;
$$ LANGUAGE plpgsql STABLE;
COMMENT ON FUNCTION partitioning.determine_partition_for_citizen(TEXT) IS '[QLDCQG-BCA] Xác định thông tin phân vùng (Miền, Tỉnh, Huyện) cho công dân dựa trên địa chỉ hiện tại.';
...
--              Hữu ích khi thông tin phân vùng của bản ghi thay đổi (vd: công dân chuyển tỉnh).
...
-- Returns: BOOLEAN (TRUE nếu thành công)
-- Requires: Hàm partitioning.determine_partition_for_citizen hoạt động chính xác (nếu dùng để lấy thông tin mới).
-- ----------------------------------------------------------------------------
...
END;
$$ LANGUAGE plpgsql;
COMMENT ON FUNCTION partitioning.move_data_between_partitions(TEXT, TEXT, TEXT, ANYELEMENT, TEXT, INTEGER, INTEGER) IS '[QLDCQG-BCA] Di chuyển một bản ghi cụ thể sang partition mới dựa trên thông tin phân vùng mới được cung cấp.';
...
--     'citizen',
--     ARRAY[
--         'CREATE INDEX IF NOT EXISTS idx_%2$s_full_name ON %1$I.%2$I USING gin (full_name gin_trgm_ops)', -- %1$I: schema, %2$I: partition name
...
BEGIN
    ...
    LOOP
        ...
        LOOP
            ...
            -- Giả định tên index nằm sau 'CREATE INDEX' hoặc 'CREATE UNIQUE INDEX' và trước 'ON'
...
END;
$$ LANGUAGE plpgsql;
COMMENT ON FUNCTION partitioning.create_partition_indexes(TEXT, TEXT, TEXT[]) IS '[QLDCQG-BCA] Tạo các index được định nghĩa trong mảng p_index_definitions trên tất cả các partition con của bảng p_table.';


-- ============================================================================
-- III. FUNCTIONS TIỆN ÍCH KHÁC (Nếu cần)
-- ============================================================================
...
BEGIN
    ...
    WITH partition_info AS (
        -- Lấy thông tin về các partition con và biểu thức phân vùng của chúng
        SELECT
            nsp.nspname AS schema_name,
            parent.relname AS table_name, -- Tên bảng cha
        ...
        WHERE child.relkind = 'r' -- Chỉ lấy các partition là bảng (relation)
          AND nsp.nspname = 'public_security' -- Chỉ xét schema public_security trong DB BCA
    ),
    partition_stats AS (
        -- Lấy thông tin thống kê và kích thước cho từng partition
        SELECT
            pi.schema_name,
            pi.table_name,
            pi.partition_name,
            pi.partition_expression,
            stat.n_live_tup + stat.n_dead_tup AS estimated_rows, -- Ước tính số dòng
            ...
            -- Trích xuất thông tin địa lý từ tên partition (giả định theo quy ước đặt tên đã dùng)
            CASE
            ...
            -- Regex phức tạp hơn để cố gắng trích xuất mã tỉnh/huyện chuẩn hóa từ tên partition
    ...
    -- Kết hợp thông tin thống kê với tên địa danh từ bảng reference
    SELECT
        ps.schema_name::TEXT,
        ps.table_name::TEXT,
        ...
        prov.province_name::TEXT, -- Lấy tên tỉnh
        dist.district_name::TEXT  -- Lấy tên huyện
    FROM partition_stats ps
    -- Join để lấy tên Tỉnh dựa trên mã code chuẩn hóa trích xuất được
    LEFT JOIN reference.provinces prov ON partitioning.standardize_name(prov.province_code) = ps.derived_province_std_code
    -- Join để lấy tên Huyện dựa trên mã code chuẩn hóa trích xuất được
...
END;
$$ LANGUAGE plpgsql STABLE;
COMMENT ON FUNCTION partitioning.generate_partition_report() IS '[QLDCQG-BCA] Tạo báo cáo về trạng thái các partition (kích thước, số dòng...) của các bảng được phân vùng trong DB BCA.';
...
BEGIN
    ...
    -- 1. Xác định các partition cần tách dựa trên p_retention_period (ví dụ: với audit_log phân vùng theo tháng)
    -- 2. Lặp qua các partition cần tách:
    --    a. Thực hiện DETACH PARTITION.
    --    b. Đổi tên bảng đã tách (ví dụ: thêm _archived).
    --    c. Có thể di chuyển bảng sang tablespace khác hoặc backup ra ngoài.
    --    d. Ghi log vào partitioning.history.
...
END;
$$ LANGUAGE plpgsql;
COMMENT ON FUNCTION partitioning.archive_old_partitions(TEXT, TEXT, INTERVAL) IS '[QLDCQG-BCA] Placeholder - Xử lý tách và lưu trữ các partition cũ (cần triển khai logic cụ thể, ví dụ cho audit_log).';

COMMIT;

\echo '-> Hoàn thành định nghĩa functions phân vùng cho ministry_of_public_security.'

-- TODO (Các bước tiếp theo):
-- 1. Execute Setup: Tạo file `execute_setup.sql` trong cùng thư mục để gọi các hàm setup (vd: `setup_nested_partitioning`) cho các bảng cụ thể.
...
-- 4. Permissions: Cấp quyền EXECUTE trên các hàm này cho role quản trị hoặc role thực thi phân vùng.