-- =============================================================================
-- File: national_citizen_central_server/schemas/audit.sql
-- Description: Tạo bảng audit_log để ghi nhật ký thay đổi dữ liệu
--              trong schema 'audit' của database Máy chủ Trung tâm (TT).
...
\echo '--> Tạo bảng audit.audit_log cho national_citizen_central_server...'
...
COMMENT ON SCHEMA audit IS '[QLDCQG-TT] Schema chứa dữ liệu nhật ký thay đổi (audit log) cho DB Trung tâm';
...
CREATE TABLE audit.audit_log (
    log_id BIGSERIAL NOT NULL,                      -- Khóa chính tự tăng của log (dùng BIGSERIAL)
    action_tstamp TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT clock_timestamp(), -- Thời điểm hành động xảy ra (Cột phân vùng)
    schema_name TEXT NOT NULL,                      -- Schema của bảng bị thay đổi (vd: 'central', 'sync')
    table_name TEXT NOT NULL,                       -- Tên bảng bị thay đổi (vd: 'integrated_citizen')
    operation cdc_operation_type NOT NULL,          -- Loại hành động (INSERT, UPDATE, DELETE - ENUM)
    session_user_name TEXT DEFAULT current_user,    -- User database thực hiện hành động (thường là sync_user hoặc central_server_admin)
    application_name TEXT DEFAULT current_setting('application_name'), -- Tên ứng dụng kết nối (vd: 'AirflowSyncJob', 'AppTT_API', 'psql')
...
COMMENT ON TABLE audit.audit_log IS '[QLDCQG-TT] Bảng ghi nhật ký chi tiết các thay đổi dữ liệu (INSERT, UPDATE, DELETE) trên các bảng được theo dõi trong database Máy chủ Trung tâm.';
...
COMMENT ON COLUMN audit.audit_log.application_name IS 'Tên ứng dụng được đặt khi kết nối (vd: AirflowSyncJob, AppTT_API, psql).';
...
CREATE INDEX IF NOT EXISTS idx_audit_log_row_data_gin ON audit.audit_log USING gin (row_data jsonb_path_ops); -- Sử dụng jsonb_path_ops
...
\echo '-> Hoàn thành tạo bảng audit.audit_log cho national_citizen_central_server.'
...
-- 2. Trigger Function: Tạo hoặc đảm bảo tồn tại hàm trigger `audit.if_modified_func()` (trong thư mục `national_citizen_central_server/triggers/audit_triggers.sql`) để ghi dữ liệu vào bảng này. Hàm này cần giống với hàm bên BCA/BTP.
-- 3. Attach Triggers: Gắn trigger `audit.if_modified_func()` vào các bảng cần theo dõi trong schema 'central' và 'sync' (nếu cần) (trong thư mục `national_citizen_central_server/triggers/audit_triggers.sql`).
-- 4. Permissions: Cấp quyền phù hợp trên bảng này cho role quản trị (central_server_admin) và có thể là role đọc đặc biệt (trong script `02_security/02_permissions.sql`). Role sync_user không nên ghi trực tiếp vào đây (việc ghi log nên do trigger đảm nhiệm).