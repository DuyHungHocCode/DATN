-- =============================================================================
-- File: national_citizen_central_server/schemas/partitioning.sql
-- Description: Tạo các bảng hỗ trợ quản lý phân vùng dữ liệu
--              trong schema 'partitioning' của database Máy chủ Trung tâm (TT).
...
\echo '--> Tạo các bảng trong schema partitioning cho national_citizen_central_server...'
...
CREATE TABLE partitioning.config (
    ...
    schema_name VARCHAR(100) NOT NULL,                -- Schema của bảng được phân vùng (vd: 'central', 'audit')
    table_name VARCHAR(100) NOT NULL,                 -- Tên bảng được phân vùng (vd: 'integrated_citizen', 'audit_log')
...
COMMENT ON TABLE partitioning.config IS '[QLDCQG-TT] Lưu trữ cấu hình phân vùng cho các bảng trong database Máy chủ Trung tâm.';
...
COMMENT ON TABLE partitioning.history IS '[QLDCQG-TT] Ghi lại lịch sử các hành động liên quan đến việc tạo, sửa, xóa, lưu trữ các partition trong database Máy chủ Trung tâm.';
...
\echo '-> Hoàn thành tạo các bảng trong schema partitioning cho national_citizen_central_server.'

-- TODO (Các bước tiếp theo liên quan đến schema này):
-- 1. Functions: Tạo các hàm quản lý phân vùng (trong thư mục `national_citizen_central_server/partitioning/`) sẽ sử dụng các bảng này.
-- 2. Configuration: Nạp dữ liệu cấu hình ban đầu vào bảng `partitioning.config` cho các bảng cần phân vùng trong DB TT (vd: integrated_citizen, integrated_household, audit_log).
